...
fn in_bounds(p: &Point, grid: &Grid) -> bool {
    p.x < 0 || p.y < 0 || p.x > (grid.columns as i32) - 1 || p.y > (grid.rows as i32) - 1
}
    ...
    for (_, points) in lot {
        for (i, p0) in points.iter().enumerate() {
            for j in 0..points.len() {
                ...
                // These are sorted due to the way we are parsing the input
                // if p0.x.abs_diff(p1.x) >= 2 || p0.y.abs_diff(p1.y) >= 2 {
